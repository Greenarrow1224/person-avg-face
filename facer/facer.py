...
def glob_image_url(face_images_list, extensions=["jpg", "jpeg", "png"]):
    """Returns a list of image files in `root`"""
    return [f for f in face_images_list if f.rsplit(".", 1)[-1] in extensions]


def load_images(root, verbose=True):
    """Returns list of image arrays
    :param root: (str) Directory containing facer images
...
# TODO 待完成
def load_request_images(imageList, verbose=True):
    """
    :param imageList: (List) facer images url
    """
    files = sorted(glob_image_url(imageList))
    num_files = len(files)
    if verbose:
        print(f"\nFound {num_files} in '{imageList}'.")
        N = max(round(0.10 * num_files), 1)
...
def load_face_landmarks(root, verbose=True):
    """Load facer landmarks created by `detect_face_landmarks()`
...
def detect_face_landmarks(images,
                          ...
                          verbose=True,
                          print_freq=0.10):
    """Detect and save the facer landmarks for each image
    ...
    :output landmarks: (list) 68 landmarks for each found facer
    :output faces: (list) List of the detected facer images
    """
    num_images = len(images.keys())
    if verbose:
        print(f"\nStarting facer landmark detection...")
        print(f"Processing {num_images} images.")
        N = max(round(print_freq * num_images), 1)

    # Look for facer landmarks in each image
    num_skips = 0
    ...
    for n, (file, image) in enumerate(images.items()):
        ...
        # Try to detect a facer in the image
...
def create_average_face(faces,
                        ...
                        verbose=True,
                        print_freq=0.05):
    """Combine the faces into an average facer"""
    if verbose:
        print(f"\nStarting facer averaging for {len(faces)} faces.")
    ...
    n = len(landmarks[0])
    w, h = output_dims
    eyecornerDst = [(np.int_(0.3 * w), np.int_(h / 3)),
                    (np.int_(0.7 * w), np.int_(h / 3))]
    ...
    if save_image:
        cv2.imwrite("../avg_images/"+output_file, 255 * output[..., ::-1])
...
def create_average_face_from_directory(dir_in,
    ...
    # Detect landmarks for each facer
    landmarks, faces = detect_face_landmarks(images, verbose=verbose)

    # Use  the detected landmarks to create an average facer
    ...
    # Save a labeled version of the average facer
    if save_image:
...
def create_animated_gif(path_to_images, save_gif=True, verbose=True):
    """Create an animated facer average GIF from a directory of images"""
    ...
    # Detect facer landmarks
    landmarks, faces = detect_face_landmarks(images, verbose=verbose)

    # Create the average facer and interim images
